...
const midReviewSchema = {
  ...
  properties: {
    ...
    suggestions: {
      type: "array",
      description:
        "If more detail could still help, include entries for those elements as Markdown with 2–4 bullets. Empty array if enough detail has been added.",
      items: {
        type: "object",
        additionalProperties: false,
        properties: {
...
${commonSuggestionRules(promptType)}
- Only include an element in "suggestions" if isSufficient=false for that element.
...
INPUT: A user's FINAL prompt after they've incorporated suggestions.
  ...
  try {
    ...
    if (action === "finalReview") {
      ...
      const response = await openai.responses.create({
        model: "gpt-4o-mini",
        input: [
          { role: "system", content: finalReviewSystemPrompt },
